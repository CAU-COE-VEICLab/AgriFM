...
def load_pretrained_agri420k(config, model, logger):
    logger.info(f"==============> Loading weight {config.MODEL.PRETRAINED} for fine-tuning......")
    ...
    if state_dict is not None:
        if config.DATA.DATASET == 'agri420k':
            ...
            logger.warning(f"agri420k training function! just support test in imagenet1k and trainig in agri420k!")
        msg = model.load_state_dict(state_dict, strict=False)
        logger.warning(msg)
        ...
        if config.DATA.DATASET == 'agri420k':
            ...
            logger.warning(f"model name {config.MODEL.TYPE} : agri420k training function! just support test in imagenet1k and trainig in agri420k!")
    logger.info(f"=> loaded successfully '{config.MODEL.PRETRAINED}'")
