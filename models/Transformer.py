...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, ff_dim, dropout, attention_dropout):
        super().__init__()
        self.attn = MultiHeadedSelfAttention(dim, num_heads, attention_dropout)
...
class Transformer(nn.Module):
    ...
    def __init__(self, num_layers, dim, num_heads, ff_dim, dropout, attention_dropout, use_checkpoint):
        super().__init__()
        self.use_checkpoint = use_checkpoint
        self.blocks = nn.ModuleList([
            Block(dim, num_heads, ff_dim, dropout, attention_dropout) for _ in range(num_layers)])

    def forward(self, x, mask=None):
        for block in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(block, x, mask)
            else:
                x = block(x, mask)
        return x
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        self.transformer = Transformer(num_layers=num_layers, dim=dim, num_heads=num_heads,
                                       ff_dim=ff_dim, dropout=dropout_rate, attention_dropout=attention_dropout_rate,
...
def test_ViT(num_classes: int = 1000, **kwargs):
    model = ViT()
    return model
