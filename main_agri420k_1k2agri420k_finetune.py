...
from utils import load_checkpoint, save_checkpoint, NativeScalerWithGradNormCount, auto_resume_helper, \
    reduce_tensor, save_checkpoint_best, load_pretrained_agri420k
...
def main(config):
    ...
    if config.MODEL.PRETRAINED and (not config.MODEL.RESUME):
        load_pretrained_agri420k(config, model_without_ddp, logger)
        acc1, acc5, loss = validate(config, data_loader_val, model)
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
        if config.EVAL_MODE:
            return
